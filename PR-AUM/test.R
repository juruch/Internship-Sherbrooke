...
#####
# Validation loss curves - Version modifiée
#####
...
# Modification : capturer toutes les métriques disponibles
...
# Premier graphique - toutes les métriques
ggplot()+
  theme_bw()+
...
# Deuxième graphique - version détaillée pour un fold
get_fold <- function(DT,it=1)DT[iteration==it]
history_fold <- get_fold(history_long)
score_fold <- get_fold(score_torch)

# Calculer les points max pour chaque métrique
max_fold <- history_fold[
  , .SD[value==max(value)]
